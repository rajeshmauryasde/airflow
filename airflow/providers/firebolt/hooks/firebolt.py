...
class FireboltHook(DbApiHook):
    ...
    def get_conn(self) -> Connection:
        """Return fireboltql connection object"""
        conn = self.get_connection(self.firebolt_conn_id)  # type: ignore
        # conn_config = {
        #     "username": conn.login,
    ...
    def run(self, sql: Union[str, list], autocommit: bool = False, parameters: Optional[dict] = None) -> None:
        ...
        with closing(self.get_conn()) as conn:
            ...
            for query in sql:
                ...
                self.log.info(query)
                print("step10")
                with closing(cursor.execute(query, parameters)) as cur:
                    print("step11")
                    print(cur)
...
#                 ),
#                 'port': 'Port',
#                 'host': 'firebolt hostname',
#                 'schema': 'firebolt schema',
#                 'login': 'firebolt username',
